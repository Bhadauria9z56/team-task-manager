...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid overflow-hidden rounded-lg border border-foreground bg-card shadow-sm md:grid-cols-[1fr_240px]">
        <div className="p-5">
          <h2 className="text-3xl font-semibold tracking-normal">{project.name}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{project.description ?? "No description yet."}</p>
        </div>
        <div className="mono-visual hidden min-h-28 border-l border-foreground md:block" />
      </div>
