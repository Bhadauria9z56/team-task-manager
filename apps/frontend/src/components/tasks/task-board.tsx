...
export function TaskBoard({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 rounded-lg border bg-card/80 p-3 shadow-sm">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {columns.map((column) => {
          ...
          return (
            <Card key={column.status} className="bg-card/80">
              <CardHeader>
              ...
              <CardContent className="space-y-3">
                {tasks.length ? (
                  ...
                  <p className="rounded-md border border-dashed bg-background/50 p-4 text-sm text-muted-foreground">
                    No tasks here.
                  </p>
                )}
              </CardContent>
...
function TaskCard({
  ...
  if (editing) {
    return (
      <form
        className="space-y-3 rounded-md border bg-background/80 p-4 shadow-sm"
        onSubmit={(event) => {
  ...
  return (
    <div className="space-y-3 rounded-md border bg-background/80 p-4 shadow-sm transition-colors hover:border-primary/40">
